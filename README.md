# Jinto Jose — Portfolio

**Head of Product · Data Domain · AI-Native Builder**

📍 The Netherlands &nbsp;·&nbsp; 🌐 [jintomjose.github.io](https://jintomjose.github.io) &nbsp;·&nbsp; 💼 [LinkedIn](https://www.linkedin.com/in/jintojose) &nbsp;·&nbsp; 🐙 [GitHub](https://github.com/jintomjose)

---

## About

I spent over 13 years in the data domain — architecting pipelines, building financial data systems, and designing data products for enterprise and financial institutions. That gave me something most product people don't have: I think in systems, not features.

From there I moved into product leadership — heading product teams, shipping roadmaps, and bridging the gap between data intelligence and business outcomes.

Now I'm in a third chapter: building AI-native applications from scratch. Claude API, Vercel, GitHub — the stack that makes a product person genuinely dangerous. I'm not just speccing products for engineers anymore. I'm shipping them myself.

---

## Projects

### AI & Full-Stack Applications

| Project | Description | Stack | Status | Links |
|---|---|---|---|---|
| **Inbox Intelligence** | End-to-end AI pipeline that surfaces what matters from your inbox | Claude API · Gmail API · Vercel | 🟡 Building | [GitHub](https://github.com/jintomjose/inbox-intelligence) · [Live](https://inbox-intelligence-testjj.vercel.app/) |
| **Encyclical Companion** | AI companion for deep reading of papal encyclicals — timeline, themes, study chat | Claude API · React/Vite · Vercel | 🟡 Building | [GitHub](https://github.com/jintomjose/encyclical-companion) · [Live](https://encyclical-companion.vercel.app/) |

### Research & Analysis

| Project | Description | Stack | Status | Links |
|---|---|---|---|---|
| **Banking AI Operating Models** | Interactive comparison of AI strategies across ING, DBS, JP Morgan, Emirates NBD, Revolut & BofA | HTML · JS · Vercel | 🟢 Live | [GitHub](https://github.com/jintomjose/banking-ai-comparison) · [Live](https://banking-ai-comparison.vercel.app/) |

### Data Products — Finance Domain

| Project | Description | Domain | Status | Links |
|---|---|---|---|---|
| **Value at Risk** | Portfolio VaR as a consumable data product — discoverable, addressable, trustworthy | Finance · Risk | Concept | [GitHub](https://github.com/jintomjose/DataProduct-Value-At-Risk) |
| **Daily PnL** | Real-time P&L as a self-describing data asset for financial decision-makers | Finance · Analytics | Concept | Private |
| **Goal: Zero Contact** | Data product to drive customer contact rates to zero via proactive intelligence | CX · Automation | Concept | Private |

### Data Products — Product Intelligence

| Project | Description | Domain | Status | Links |
|---|---|---|---|---|
| **Innovation Index for Dev Teams** | Turns "are we innovative?" into a measurable Product Owner KPI | Org Intelligence | Concept | [GitHub](https://github.com/jintomjose/DataProduct-Innovation-Index-4-DevTeams) |

### Web Applications

| Project | Description | Stack | Status | Links |
|---|---|---|---|---|
| **Momentum** | Habit tracker with one rule: only 3 habits. Constraint as product design. | Base44 | 🟢 Live | [GitHub](https://github.com/jintomjose/WebApp-momentum.app) · [Live](https://momentum-3df5dd58.base44.app/home) · [Instagram](https://www.instagram.com/momentum.app.3) |
| **ReginaCaeliCreations** | E-commerce site for a home baker — real product problem, real user | Base44 | 🟢 Live | [GitHub](https://github.com/jintomjose/WebApp-Annus-cakes.app) · [Live](https://annus-cakes-0eadb5fc.base44.app) · [Instagram](https://www.instagram.com/ReginaCaeliCreations) |

---

## Skills

**Product** — Data Product Design · Roadmapping · OKRs · Product Owner KPIs · Agile · Product-led Growth

**Technical** — Claude API · React · Next.js · Vite · Vercel · GitHub Actions · HTML/CSS/JS · Data Pipelines · Financial Data Systems

**Domains** — Financial Services · AI/ML · Customer Experience · Engineering Intelligence

---

## By the numbers

| | |
|---|---|
| 13+ | Years in the data domain |
| 4 | Active builds in progress |
| 11 | Total projects |
| 5 | Live deployments |
| 3 | Domains covered |

---

## Contact

- 🌐 Portfolio: [jintomjose.github.io](https://jintomjose.github.io)
- 💼 LinkedIn: [linkedin.com/in/jintojose](https://www.linkedin.com/in/jintojose)
- 🐙 GitHub: [github.com/jintomjose](https://github.com/jintomjose)

---

*For machine-readable structured data, see [ai-readable.md](./ai-readable.md) and [projects.json](./projects.json)*
